...
const StudentTravelTable = () => {
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      const response = await fetch("/api/asiacrypt-visa-request/export?type=1");
      ...
      if (result.success && result.data) {
        ...
        // 添加工作表到工作簿
        XLSX.utils.book_append_sheet(workbook, worksheet, "Student Travel");

        // 生成文件名（包含当前日期）
        const now = new Date();
        const dateStr = now.toISOString().split("T")[0]; // YYYY-MM-DD格式
        const filename = `student_travel_${dateStr}.xlsx`;
...
export default StudentTravelTable;
